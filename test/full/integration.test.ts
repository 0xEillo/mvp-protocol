  ...
  describe("full", function () {
    ...
    it("Should vote multiple times for different candidates and check the leaderboard", async function () {
      ...
      accountList.forEach(async (element) => {
        await token.connect(element).claim(element.address);
      });
      expect(
      ...
      await votesGovernor.connect(acc1).vote(1, 4);
      ...
      await votesGovernor.connect(acc5).vote(2, 1);
      await votesGovernor.connect(acc6).vote(2, 1);
      await votesGovernor.connect(acc7).vote(2, 1);
      await votesGovernor.connect(acc8).vote(3, 1);
